...
describe('buildObsidianURL', () => {
	...
	const expected = new URL('obsidian://new')
	const defaultFileName = 'foo'
	const defaultFileContent = '# Hello'

	beforeEach(() => {
		expected.searchParams.delete('name')
		expected.searchParams.delete('content')
		expected.searchParams.delete('vault')
	})
	...
	test.each([
		...
		({folder = defaultFolder, fileContent, fileName, vault}) => {
			expected.searchParams.set(
				'name',
				encodeURIComponent(`${folder}${fileName}`),
			)
			expected.searchParams.set('content', encodeURIComponent(fileContent))
			if (vault) {
				expected.searchParams.set('vault', vault)
			}
